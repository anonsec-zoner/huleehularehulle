...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id == bot.id:
        message.reply_text(tld(chat.id, "I'm not gonna BAN myself, are you high?"))
        return ""
    ...
    try:
        ...
        message.reply_text(tld(chat.id, "Fun isn’t something one considers when balancing the universe.But this… does put a smile on my face."))
        return log
        ...
        if excp.message == "Reply message not found":
            # Do not reply
            #bot.send_sticker(chat.id, BAN_STICKER)  # banhammer marie sticker
            message.reply_text(tld(chat.id, "Fun isn’t something one considers when balancing the universe.But this… does put a smile on my face."), quote=False)
            return log
        else:
            LOGGER.warning(update)
            LOGGER.exception("ERROR banning user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
                             excp.message)
            message.reply_text(tld(chat.id, "Well damn, I can't ban that person."))
...
def temp_ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member):
        message.reply_text(tld(chat.id, "This man is under my protection, meaning that you cannot ban this man! 😏"))
        return ""
    ...
    try:
        ...
        #bot.send_sticker(chat.id, BAN_STICKER)  # banhammer marie sticker
        message.reply_text("Banned! Person will be banned for {}.".format(time_val))
        return log
...
def kick(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if is_user_ban_protected(chat, user_id):
        message.reply_text("Why would I kick an adminr? That sounds like a pretty dumb idea.")
        return ""

    res = chat.unban_member(user_id)  # unban on current user = kick
    if res:
        #bot.send_sticker(chat.id, BAN_STICKER)  # banhammer marie sticker
        message.reply_text("Insect get lost!")
        log = "<b>{}:</b>" \
              "\n#KICKED" \
              "\n<b>Admin:</b> {}" \
